...
public class TabToFragment extends Fragment {
    ...
    private void getSelectedDate() {
        ...
        calendarView.setEventHandler(new CalendarView.EventHandler() {
            ...
            public void onItemClick(Date date) {
                DateFormat df = SimpleDateFormat.getDateInstance();
                ((MainForDateActivity) getActivity()).getSelectedDateto(df.format(date));
                Toast.makeText(getActivity(), df.format(date), Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void initView(View view) {
        calendarView = (CalendarView) view.findViewById(R.id.fragment_tab_from_for_date_picker_start_and_end_calendar_view);
    }
}
